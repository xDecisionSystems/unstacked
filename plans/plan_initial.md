...
Remaining: final REST/OpenAPI validation against a real action client (T9.3). Descriptor-confined lifecycle I/O (T2.1) is now complete for every content operation, including recursive container delete/rename. The browser UI, browser search, backup console, operator documentation, and ACL/path test suite are complete. MCP is deliberately out of scope: the REST API is the only AI transport.
