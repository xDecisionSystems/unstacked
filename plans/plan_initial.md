...
**Remaining:** Descriptor-confined read/no-clobber creation helpers and confined `.pages` control-file I/O now protect page creation, updates, title edits, delete, and move (including their rollback/navigation rewrites) from symlink races. Remaining: recursive container delete/rename and their navigation rewrites still need a transaction-wide descriptor-based I/O refactor before this task can be complete.
