...
**Every planned task is now `[x]` or `[not planned]` (MCP, dropped by user decision).** This is not the same claim as "production-hardened and audited" — it means every task's own done-when criteria are met and independently verified by whichever agent (this session or Codex) merged it, per the per-task notes below. Descriptor-confined lifecycle I/O (T2.1) covers every content operation including recursive container delete/rename; the AI surface (T9.3) has its own scoped, validated OpenAPI document separate from the full app schema; the browser UI, browser search, backup console, operator documentation, and ACL/path test suite are all complete. Two coordination collisions occurred during development (documented below and in this file's own history) where this session and Codex independently built the same task unaware of each other — worth knowing before assuming every commit message reflects a clean, uncontested implementation path.
...
**Note:** The last gap — recursive container delete/rename — is closed. `ConfinedTree` gained `walk_files` (recursive, descriptor-confined subtree listing, used to snapshot before a delete and to build a rename's affected-path list without ever resolving a `Path`). `_delete_container`/`_rename_container` in `app/content.py` no longer touch `shutil.rmtree`/`os.replace`/`safe_join` at all — every read, the recursive delete, the atomic no-clobber directory rename, and rollback now go through `ConfinedTree`/`_ConfinedRollback` exclusively (verified by grep: zero matches for those three calls in either method). Rename's rollback doesn't need a byte-snapshot the way delete's does — a directory rename is one atomic operation, so undoing it is just the reverse rename, tracked via a small ordered stack of undo closures (source rename, asset rename, content-rewrite, each unwound in reverse) rather than forcing a directory move through the single-file rollback model. The dead Path-based `_changed_nav` (superseded by `_changed_nav_confined`, which now also serves the container case via a `parent: str | None` widening — `None` means the docs root, which is a book's own "parent") and `_container_path` were removed rather than left unused.
Verified, not just asserted: added two adversarial tests (`tests/test_content_symlink_races.py`) that swap the parent book for a symlink *after* the confined subtree walk but *before* the delete/rename executes, for both `delete_chapter` and `rename_chapter` — both correctly raise and leave the outside sentinel untouched. Confirmed by a throwaway mutation test that reverting the delete path to raw `shutil.rmtree` still passed those two tests (an earlier confined step — the nav-parent check — independently catches the same swap first), so also added a call-spy proving `ConfinedTree.delete_tree`/`.rename()` are genuinely invoked with the expected arguments, not merely that the pipeline happens to be safe via a different layer. Full suite 585 passing (was 583), ruff clean.
...
#### [x] T9.3 — REST + OpenAPI surface (ChatGPT) **[P]**
...
**Note:** Closed the last gap — real OpenAPI validation. `app/ai_api.py` exposes auth, tree, search, content, export, history, diff, restore and create endpoints with bearer auth, bounded page/diff responses, and a dedicated per-user AI rate limit.
A real discovery along the way: the whole app's `/openapi.json` mixes the AI surface with the admin console, backup, and browser-cookie routes — 40 operations across 32 paths, comfortably over the 30-operation limit ChatGPT Actions enforces on one imported schema, and inappropriate to expose as "tools" to an AI client regardless of the limit. Added a second, purpose-built document at `GET /api/ai/openapi.json` (`build_ai_openapi_schema` in `app/ai_api.py`), built directly from the router's own route objects filtered to `/api/ai/*` — 13 paths, 14 operations — so it can never drift from what those routes actually accept. `/api/auth/token` and `/api/auth/tokens/revoke` are deliberately excluded: an Action is configured with one bearer token obtained out of band, not by calling a credential-issuing endpoint as one of its own operations. Added `Settings.public_base_url` (optional; validated as an absolute http(s) URL, no query string, trailing slash stripped) so the schema can declare the one `servers` entry an Action needs to know where to call — omitted rather than guessed when unset.
Validated with the real `openapi-spec-validator` library against the actual OpenAPI 3.1 JSON Schema (not a shape this app's own tests invented), plus explicit ChatGPT-Action-specific checks a generic validator wouldn't catch: unique `operationId`s, the 30-operation ceiling, and — mutation-tested, not assumed — that every operation actually carries the `HTTPBearer` security requirement (confirmed by stripping one operation's `security` array and observing the check fail while the generic OpenAPI validator still accepted the now-unauthenticated shape as structurally valid). Full suite 602 passing (was 594 before this task), ruff clean.
...
| AI access | Read/export and create operations through one `ai_service` behind REST only, no MCP | REST inherits the same ACL and Git mutation path; book/chapter creation is admin-only and page creation requires parent write access. MCP dropped (2026-08-28, user decision) — it costs materially more tokens per call than a plain API for the same operations. |
