...
def install(app: FastAPI) -> None:
    """Register the lifecycle hooks and activate an already-configured target.

    Called once from ``create_app``.  The hooks are registered whether or not a
    target exists, because one may be configured later in this process's life;
    both are no-ops when nothing is wired.
    """
    ...
    if effective_target(app.state.settings).configured:
        activate(app)

    @app.on_event("startup")
    def start_backup_services() -> None:
